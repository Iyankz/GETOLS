...
class OLTService:
    ...
    def create(
        ...
        olt = OLT(
            ...
            cli_password_encrypted=encrypt_credential(cli_password),
    ...
    def update(
        ...
        if cli_username is not None:
            olt.cli_username = cli_username
        
        if cli_password is not None:
            olt.cli_password_encrypted = encrypt_credential(cli_password)
    ...
    def test_cli_connection(self, olt_id: int) -> Tuple[bool, Optional[str]]:
        """
        Test CLI connection to OLT via Telnet.
        ...
        try:
            # Decrypt credentials
            password = decrypt_credential(olt.cli_password_encrypted)
            ...
            # Update test result
            olt.last_test_cli = datetime.utcnow()
            olt.last_test_cli_success = success
            self.db.commit()
            ...
            olt.last_test_cli = datetime.utcnow()
            olt.last_test_cli_success = False
            self.db.commit()
            return False, str(e)
    ...
    def get_cli_adapter(self, olt: OLT):
        """
        Get CLI adapter for an OLT.
    ...
    def get_snmp_rw_manager(self, olt: OLT) -> Optional[SNMPManager]:
        ...
        Used for maintenance operations (ONU reset, etc).
